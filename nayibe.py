...
#fondo
fondo=pg.image.load("./imagenes/fondo space.png").convert()
posicion_fondo=(0,0)
#fondo particulas
...
#variables player
nave_player=pg.image.load("./imagenes/NAVE 2.png").convert_alpha()
    ...
    for event in pg.event.get():
        ...
        if event.type==pg.KEYDOWN:
            if event.key==pg.K_LEFT:
                speed_playerx=-speed_nave
            if event.key==pg.K_RIGHT:
                speed_playerx=speed_nave
            if event.key==pg.K_UP:
                speed_playery=-speed_nave
            if event.key==pg.K_DOWN:
                speed_playery=speed_nave
            if event.key==pg.K_SPACE:
                disparos.append(imagen_bala.get_rect(midbottom=rectangulo_player.center))
